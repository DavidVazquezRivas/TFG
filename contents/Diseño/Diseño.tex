...
Como se mencionó en la definición de la arquitectura, la separación física y lógica de las bases de datos por cada \emph{Bounded Context} impone el reto de mantener la consistencia de la información sin recurrir a transacciones distribuidas, las cuales penalizarían gravemente el rendimiento y la disponibilidad del sistema. Para resolver este desafío, se ha implementado una arquitectura basada en eventos combinada con el patrón \emph{Transactional Outbox} y su contraparte \emph{Inbox}.
...
\begin{enumerate}
    ...
    \item \textbf{Fase 2: Ejecución e Invocación del Modelo:} Un proceso en segundo plano (\emph{webjob}) recoge las ejecuciones pendientes. El sistema compone el contexto, inyectando el \emph{system prompt} del agente, la información del entorno y el historial de ejecución, y realiza la llamada al \ac{LLM}. A partir de aquí, la \ac{IA} razona sobre los pasos a seguir. Si decide ejecutar una acción directa, invoca la \ac{API} de \emph{Tools}, procesa el resultado y continúa. Sin embargo, si el agente determina que necesita información o aprobación de un humano, invoca la herramienta \texttt{SuspendWorkflow}. Esta acción actualiza el estado de la ejecución a suspendida, guarda el contexto de la conversación para no perder memoria de la misma, e inserta un registro indicando qué evento específico está esperando para reactivarse.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Un \emph{Access Token} (en formato \ac{JWT}) de vida corta (60 minutos), que se devuelve en el cuerpo de la respuesta HTTP. El \emph{frontend} almacena este token \textbf{exclusivamente en memoria} (nunca en \emph{Local Storage}), protegiéndolo contra extracciones maliciosas por parte de \emph{scripts} de terceros.
    ...
    Al realizar esta petición, el navegador incluye automáticamente la cookie \texttt{HttpOnly} con el \emph{Refresh Token}. El \emph{backend} recibe la petición, valida la existencia y vigencia del token en su base de datos, e implementa un mecanismo de \textbf{Rotación de Tokens de Refresco} (\emph{Refresh Token Rotation}):
    \begin{itemize}
